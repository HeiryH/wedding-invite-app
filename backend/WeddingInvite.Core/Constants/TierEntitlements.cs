...
{
    /// <summary>
    /// Single source of truth for tier ordering (BASIC / PREMIUM / PRO — renamed from FREE since
    /// there is no longer a genuinely free tier). Tier is still set manually by an admin (no
    /// billing). Which *features* each tier unlocks now lives in the Package/PackageFeature
    /// tables (see IPackageRepository.TierIncludesFeatureAsync) — the BASIC/PREMIUM/PRO packages
    /// *are* the tier definitions, editable at /super-admin/packages.
    ...
    public static class TierEntitlements
    {
        ...
        /// <summary>Numeric rank of a tier string; unknown/blank tiers rank as BASIC (0).</summary>
