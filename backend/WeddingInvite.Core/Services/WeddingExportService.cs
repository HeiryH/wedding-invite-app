...
{
    public class WeddingExportService : IWeddingExportService
    {
        ...
        public async Task<byte[]?> BuildExportZipAsync(int weddingId)
        {
            ...
            {
                var template = await _templateService.GetByIdAsync(wedding.TemplateId);
                var owner = await _userRepo.GetByWeddingIdAsync(weddingId); // couple admin; PasswordHash excluded below
                ...
                var guests = (await _guestService.GetByWeddingIdAsync(weddingId, null, null)).ToList();
                WriteCsvEntry(zip, "guests.csv",
                    ...
                    {
                        g.GuestName, g.Email, g.PhoneNumber, g.BrideOrGroomSide,
                ...
                var itinerary = await _itineraryService.GetByWeddingIdAsync(weddingId);
                ...
                var photos = (await _photoRepo.GetAllByWeddingIdAsync(weddingId)).ToList();
