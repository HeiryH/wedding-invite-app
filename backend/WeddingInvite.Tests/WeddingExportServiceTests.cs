...
/// <summary>
/// WeddingExportService bundles every piece of a wedding's data (RSVPs, wishes, seating, itinerary,
/// full customization config, photos, audio) into one zip. These tests pin: null on a missing
/// wedding, the expected top-level entries, graceful handling of missing on-disk files (no real
/// filesystem exists in this test process), and correct CSV escaping.
/// </summary>
public class WeddingExportServiceTests
{
    private const int WeddingId = 1;

    private static WeddingExportService BuildService(TestDb db) => new(
        new WeddingRepository(db.Context),
        new UserRepository(db.Context),
        new TemplateService(new TemplateRepository(db.Context)),
        new GuestService(new GuestRepository(db.Context), new WeddingRepository(db.Context)),
        new WishService(new WishRepository(db.Context), new WeddingRepository(db.Context)),
        new TableService(new TableRepository(db.Context), new GuestRepository(db.Context), new WeddingRepository(db.Context)),
        new ItineraryService(new ItineraryRepository(db.Context), new WeddingRepository(db.Context)),
    ...
    public async Task BuildExportZipAsync_ProducesExpectedTopLevelEntries()
    {
        ...
        db.Context.Guests.Add(new Guest { WeddingId = WeddingId, GuestName = "Guest 1", BrideOrGroomSide = "Bride" });
        db.Context.Wishes.Add(new Wish { WeddingId = WeddingId, GuestName = "Guest 1", Message = "Congrats!" });
        db.Context.Tables.Add(new Table { WeddingId = WeddingId, TableName = "Table 1", Capacity = 8 });
        db.Context.ItineraryItems.Add(new ItineraryItem { WeddingId = WeddingId, Label = "Ceremony", SortOrder = 1 });
        db.Context.TemplateConfigs.Add(new WeddingTemplateConfig { WeddingId = WeddingId, ConfigKey = "invite.heading", ConfigValue = "Welcome" });
        db.Context.SaveChanges();
    ...
    {
        using var db = SeedWedding();
        db.Context.TemplateConfigs.Add(new WeddingTemplateConfig
        {
            WeddingId = WeddingId,
            ConfigKey = "music.url",
