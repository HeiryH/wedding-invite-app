...
The rose-ringed hourglass sits in the midground band, horizontally centred,
...
Centred beneath, in order with generous spacing:
  ...
  a vertical schedule list of four time-and-event rows, each a time in small caps on the left and an event name beside it [itineraryList]
...
Confirmed props for this section (spec/assets.json) — no others may appear:
  ...
  - Baby's breath sprig — band: foreground-frame, 0.3x anchor (prop)
  - Rose-ringed hourglass — band: midground, 0.6x anchor (icon)

Geometry (spec/scene.json): scale anchor = a fully-open rose bloom at
18% of frame height at band multiplier 1.0;
bands {"background": 2.2, "midground": 1.0, "foreground-frame": 1.4}; reserved content zone [33, 78]%.
