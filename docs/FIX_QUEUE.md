...
`CLAUDE.md`, pipeline scratch, and untracked-but-present junk. This is the inventory to walk
through **before** deleting anything — every row is a candidate, not a decision. Measured on
`ae-unified` at `28bd82d`.

### Tracked in git (these cost every clone)

| Path | Size | What it is | Evidence / notes |
|---|---|---|---|
| `docs/rose-horizon-art/` | **103 MB**, 46 files | T11 art pipeline working files: 6 flowbg seam-test PNGs (4.5–9.9 MB each), per-section `background.png` masters | Nothing under `frontend/` references `docs/`; the shipped art is `public/templates/rose-horizon/`. The 6 seam tests are throwaway experiments. |
| `docs/rose-horizon-props/` | **23 MB**, 33 files | T11 prop sheets + `reassembly.png` | Same — pipeline intermediates, shipped props live in `public/`. |
| `frontend/docs/rose-horizon-qa/` | 8.9 MB, 12 files | T11 QA screenshots | Point-in-time evidence; T11 has shipped. |
| `docs/rose-horizon-refs/`, `docs/rose-horizon-design-brief.md` | 112 KB | T11 reference + brief | Small; keep-or-go together with the above. |
| `docs/invite-pipeline.md` | 93 KB | Hermes kanban pipeline spec for authoring templates | Live process doc, owned by Hermes — probably keep, but confirm it's the canonical copy. |
| `docs/MULTI_VERTICAL_STRATEGY.md` | 12 KB | Strategy doc (Jul 2026) for multi-vertical | Phase 1 (Wedding→Event rename) is done; check what's still forward-looking vs. already in `CLAUDE.md`. |
| `spec/` (`background.md`, `sections/00–06`) | 80 KB, 11 files | "Gate 2" section specs for the rose-horizon flow background | Pipeline artefact for one template; `tools-flowbg/write-section-specs.py` generates them. |
| `tools-flowbg/` | 68 KB, 7 scripts | T11 flow-background tooling (`flowbg.py`, `reassemble.py`, crop fixes) | One-off pipeline scripts; `__pycache__/` beside them is untracked but not ignored. |
| `CUSTOM_DOMAINS.md` + `Caddyfile` | 3.5 KB + 1.5 KB | Caddy on-demand-TLS plan | **Never deployed** — prod is Nginx Proxy Manager (see Quick Deploy in `CLAUDE.md`). Neither is referenced by `docker-compose.yml` or `scripts/`. Actively misleading. |
| `nginx.conf` | 1.4 KB | Old nginx config (May) | Not referenced by compose either; prod's nginx lives in the NPM container. |
| `GO_LIVE.md` (102 lines), `DEPLOYMENT.md` (307 lines) | | Runbooks from May/Jul | Overlap each other and the `CLAUDE.md` Quick Deploy section; `DEPLOYMENT.md` predates the NPM switch and the `theinvit-e` domain. Worth one merged, current runbook. |
| `frontend/docs/TEMPLATE6_CUSTOMIZATION.md`, `TEMPLATE6_PIXEL_ART.md` | 43 KB | T6 build notes (Jul) | Only `CLAUDE.md` mentions them by name; check whether anything is still true post-engine. |
| `frontend/docs/design-system.md` | 20 KB | Admin UI token reference | Probably keep. |
| `frontend/public/{file,globe,next,vercel,window}.svg` | | `create-next-app` boilerplate | Unused. Safe. |
| `frontend/README.md` | | `create-next-app` boilerplate README | Unused. |
| `frontend/public/template-previews/` | 12 PNGs | Picker thumbnails | Keep — but they're regenerated by `scripts/generate-template-previews.mjs`; `classic-rose`, `engraved-certificate`, `gilded-arch`, `golden-elegance`, `minimal-noir`, `garden-romance` are the T1–T6/T8/T9 codes — verify each still maps to a live template row. |
| `docs/FIX_QUEUE.md` | 66 KB | this file | Issues 1–2, 4–6 are FIXED; consider moving closed issues to an archive section so the open queue is readable. |

### Untracked but sitting in the tree (not in `.gitignore`)

| Path | Size | Notes |
|---|---|---|
| `.worktrees/` (4 worktrees) | large | `dreamy-woodland`, `rose-horizon`, `sandy-beach` are all merged — `git worktree remove` them; `dino-doodle-party` still holds a 1.7 GB `.einvite-run/`. Add `.worktrees/` to `.gitignore`. |
| `.worktrees/dino-doodle-party-20260915/.einvite-run/` | **1.7 GB** | Pipeline run scratch (candidates/derived/extracted). Add `.einvite-run/` to `.gitignore` before anyone `git add -A`s it. |
| `docs/sandy-beach-probe/` | 9 MB | T14 probe (refs/spec/welcome). Same class as the rose-horizon dirs — decide the rule once for all of them. |
| `docs/sunny-safari-reference/` | empty dir | Delete. |
| `AGENTS.md` | 54 KB | Codex mirror of `CLAUDE.md`, drifting (last touched 09-15, `CLAUDE.md` 09-21). Either generate it or drop it. |
| `.codex/`, `.hermes/`, `.claude/` | small | Per-tool config; ignore-list them explicitly. |
| `wedding.db` (repo root, **0 bytes**) and `frontend/wedding.db` | | Stray empty files — `CLAUDE.md` already warns the root one is a decoy for the real `/opt/wedding-app/data/db/wedding.db`. Delete both and add `*.db` to `.gitignore` (the `.bak-*` rule at line 28 already assumes this). |
| `frontend/tsconfig.tsbuildinfo` | | Build cache, not ignored. |
| `**/__pycache__/`, `**/.DS_Store` | | `.DS_Store` is ignored; `__pycache__` is not. |

### Suggested rule to decide the art-working-file rows

Pick one and apply it to rose-horizon / sandy-beach-probe / any future template alike:
- **(a)** Working files never live in the repo — move to the pipeline's own storage (`.einvite-run`
  or the Obsidian vault) and keep only the design brief + a README pointing there; or
- **(b)** Keep a *small* `docs/<template>/` with brief + refs + final composited plates
  (< 5 MB), and drop intermediates (seam tests, prop sheets, QA screenshots).
