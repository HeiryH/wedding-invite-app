...
export default function HostAdminDashboard() {
  ...
  const fetchWeddings = async () => {
    try {
      setLoading(true);
      setWeddings(await eventService.getMine());
    } catch (err: any) {
  ...
  const filtered = useMemo(() => weddings.filter(w => {
    ...
    return true;
  }), [weddings, filter, eventTypeFilter, searchTerm]);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 24 }}>
        ...
        <div className="hidden lg:block">
          <Button
            ...
            onClick={() => router.push('/host-admin/wedding/create')}
          >
            New event
          </Button>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 20 }} className="sm:grid-cols-4">
        <StatCard label="Total events" value={counts.all} icon={<Icon name="heart" size={16} />} tone="brand" />
      ...
      {filtered.length > 0 ? (
        <div style={{ display: 'grid', gap: 12 }} className="md:grid-cols-2 xl:grid-cols-3">
          {filtered.map(w => (
            <WeddingCard
              key={w.eventId}
              wedding={w}
              onManage={id => router.push(`/host-admin/wedding/${id}`)}
        ...
        <EmptyState
          ...
          action={!searchTerm ? (
            <Button variant="primary" tone="brand" iconLeft={<Icon name="plus" size={15} />} onClick={() => router.push('/host-admin/wedding/create')}>
              New event
            </Button>
          ) : undefined}
        />
