  ...
  const fetchWeddingAndWishes = async () => {
    try {
      ...
      const weddingResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/event/slug/${coupleName}`
      );
      const wedding = await weddingResponse.json();

      // Wrong type prefix for a real event -> redirect to the canonical one.
      const correctSegment = urlSegmentForEventType(wedding.eventType);
      if (correctSegment !== urlEventType) {
        redirected = true;
        router.replace(`/${correctSegment}/${coupleName}/wishes`);
        return;
      }

      setWeddingId(wedding.eventId); // Cache event ID

      // Pre-existing bug found while wiring this route: this previously read
      // `wedding.weddingId`, which doesn't exist on the fetched EventDto (the field is
      // `eventId`) — so this call always fetched wishes for `undefined`.
      const wishesData = await wishService.getByWeddingId(wedding.eventId);
