...
import { guestService, CreateGuest, Event } from '@/lib/api';
...
export default function RSVPPage() {
  ...
  const [weddingData, setWeddingData] = useState<Event | null>(null);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/event/slug/${coupleName}`)
      .then((r) => r.json())
      .then((w) => {
        setWeddingData(w);
        // Wrong type prefix for a real event -> redirect to the canonical one.
        const correctSegment = urlSegmentForEventType(w.eventType);
        if (correctSegment !== urlEventType) {
          router.replace(`/${correctSegment}/${coupleName}/rsvp`);
        }
      })
      .catch(() => {});
  ...
  const onSubmit = async (data: RSVPFormData) => {
    try {
      ...
      const guestData: CreateGuest = {
        ...
        guestSide: data.brideOrGroomSide === 'Bride' ? 'PRIMARY' : 'SECONDARY',
        songRequest: data.songRequest,
        isAttending,
        numberOfAttendees: Number(data.numberOfAttendees),
      };

      await guestService.rsvp(wedding.eventId, guestData);

      setSubmitSuccess(true);

      setTimeout(() => {
        router.push(`/${urlEventType}/${coupleName}/wishes`);
      }, 2000);
    } catch (err: any) {
      setSubmitError(
