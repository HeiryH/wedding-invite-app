...
import { eventService, photoService, eventFeatureService, Event, Photo } from '@/lib/api';
...
export default function PhotoBoothPage() {
  ...
  const coupleName = params.slug as string;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const correctSegment = urlSegmentForEventType(weddingData.eventType);
        if (correctSegment !== urlEventType) {
          redirected = true;
          router.replace(`/${correctSegment}/${coupleName}/photos`);
          return;
        }
        ...
        const isEnabled = await eventFeatureService.isFeatureEnabled(
          weddingData.eventId,
          'PHOTO_BOOTH'
        );
        ...
        if (isEnabled) {
          // Fetch photos
          const photosData = await photoService.getVisibleByWeddingId(weddingData.eventId);
    ...
    fetchData();
  }, [urlEventType, coupleName, router]);
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Refresh photos
      const updatedPhotos = await photoService.getVisibleByWeddingId(wedding.eventId);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push(`/${urlEventType}/${coupleName}`)}
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-gray-600 text-lg">
            Share your favorite moments from {wedding?.displayName}'s special day!
          </p>
          <button
            onClick={() => router.push(`/${urlEventType}/${coupleName}`)}
