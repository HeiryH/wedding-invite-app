...
import { T6_STAGES, T6_ASSETS } from './data/t6Stages';
...
const ENGINE: Engine = { assetRoot: T6_ASSETS, assetSizes: {}, slotRegistry: {} };

/**
 * Renders the decorative layer overlay for one Template 6 section. Reads the couple's
 * `t6.layout.<breakpoint>.<stageId>` delta straight from customConfig (the Adjust dock in the
...
export default function SectionOverlay({
  ...
  const def = T6_STAGES[stageId];
  const { layers } = useMemo(
    () => resolveStage('t6', def, breakpoint, config),
