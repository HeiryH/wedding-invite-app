...
export const T11_STAGES: Record<string, StageDef> = {
  welcome: stage('welcome', 'Welcome', [
    anchor('themeLabel', 'Theme Label', { styleable: true }),
    ...
    anchor('nameFirst', 'First Name', { styleable: true }),
    anchor('nameSecond', 'Second Name', { styleable: true }),
    anchor('date', 'Date', { styleable: true }),
    anchor('timer', 'Countdown Timer', { styleable: true }),
    anchor('hijri', 'Hijri Date', { styleable: true }),
    anchor('venue', 'Venue', { styleable: true }),
  ]),
  walimah: stage('walimah', 'Ceremony', [
    anchor('title', 'Ceremony Title', { styleable: true }),
  ]),
  ...
  rsvp: stage('rsvp', 'RSVP', [
    slot('rsvpTitle', 'rsvpTitle', 'RSVP Title', { order: 0, z: 5 }),
    slot('rsvpPrompt', 'rsvpPrompt', 'RSVP Prompt', { order: 1, z: 4 }),
    slot('rsvpForm', 'rsvpForm', 'RSVP Form · in pop-up', { order: 2, z: 3, presentation: 'sheet', sheetId: 'rsvp' }),
    slot('rsvpSeating', 'rsvpSeating', 'Seating · in pop-up', { order: 2, z: 2, presentation: 'sheet', sheetId: 'rsvp' }),
    slot('rsvpTrigger', 'sheetTrigger', 'RSVP Button', { order: 3, z: 1, sheetId: 'rsvp', animIdle: 'wave', animIdleSpeed: 0.75 }),
  ]),
  itinerary: stage('itinerary', 'Schedule', [
    slot('itineraryTitle', 'itineraryTitle', 'Schedule Title', { order: 0, z: 3 }),
    slot('itineraryList', 'itineraryList', 'Schedule List', { order: 1, z: 2 }),
  ...
  wishes: stage('wishes', 'Wishes', [
    slot('wishTitle', 'wishTitle', 'Wishes Title', { order: 0, z: 6 }),
    slot('wishPrompt', 'wishPrompt', 'Wishes Prompt', { order: 1, z: 5 }),
    slot('wishForm', 'wishForm', 'Wish Form · in pop-up', { order: 2, z: 4, presentation: 'sheet', sheetId: 'wish' }),
    slot('wishPhoto', 'wishPhoto', 'Add a Photo · in pop-up', { order: 2, z: 3, presentation: 'sheet', sheetId: 'wish' }),
    slot('wishTrigger', 'sheetTrigger', 'Write a Wish Button', { order: 3, z: 2, sheetId: 'wish', animIdle: 'wave', animIdleSpeed: 0.75 }),
    slot('wishList', 'wishList', 'Wishes List', { order: 4, z: 1 }),
  ]),
  photobooth: stage('photobooth', 'Photos', [
    ...
    slot('photoBooth', 'photoBooth', 'Photo Booth', { order: 0, z: 1 }),
  ]),
};
