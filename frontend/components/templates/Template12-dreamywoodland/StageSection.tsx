...
export function StageSection({ code, children, extra, breakpoint, config, editor, panelless }: {
  code: SectionCode;
  children: ReactNode;
  extra?: ReactNode;
  ...
  return (
    <section ref={sectionRef} id={code} data-stage={code} data-seen={editing || seen ? 'true' : 'false'} className={styles.section}>
      ...
      <div className={panelless ? styles.flowZone : styles.panel} data-depth={0.22}>{children}</div>
      {extra}
    </section>
  );
}
