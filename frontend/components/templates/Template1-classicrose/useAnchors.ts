...
/**
 * Turns Template 1's `anchor` layers into an inline style to merge onto the real DOM element they
...
export function useAnchors(
  ...
  const byStage = useMemo(() => {
    ...
    for (const id of Object.keys(T1_STAGES)) {
      map[id] = resolveStage('t1', T1_STAGES[id], breakpoint, config).layers;
    }
    return map;
  }, [config, breakpoint]);
