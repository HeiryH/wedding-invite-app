...
import { T1_STAGES, T1_ASSETS } from './data/t1Stages';
...
const ENGINE: Engine = { assetRoot: T1_ASSETS, assetSizes: {}, slotRegistry: {} };

/**
 * Renders the decorative layer overlay for one Template 1 section. Reads the couple's
 * `t1.layout.<breakpoint>.<stageId>` delta straight from customConfig (the Adjust dock in the
...
export default function SectionOverlay({
  ...
  const def = T1_STAGES[stageId];
  const { layers } = useMemo(
    () => resolveStage('t1', def, breakpoint, config),
