...
/**
 * Turns Template 4's `anchor` layers into an inline style to merge onto the real DOM element they
 ...
 * framer-motion anchor target (e.g. the hero heading) is instead wrapped in a plain outer element
 * that carries this style, leaving the inner motion node's own animation untouched.
 *
...
export function useAnchors(
  ...
  const byStage = useMemo(() => {
    ...
    for (const id of Object.keys(T4_STAGES)) {
      map[id] = resolveStage('t4', T4_STAGES[id], breakpoint, config).layers;
    }
    return map;
  }, [config, breakpoint]);
