...
import { T2_STAGES, T2_ASSETS } from './data/t2Stages';
...
const ENGINE: Engine = { assetRoot: T2_ASSETS, assetSizes: {}, slotRegistry: {} };

/**
 * Renders the decorative layer overlay for one Template 2 section. Reads the couple's
 * `t2.layout.<breakpoint>.<stageId>` delta straight from customConfig (the Adjust dock in the
...
export default function SectionOverlay({
  ...
  const def = T2_STAGES[stageId];
  const { layers } = useMemo(
    () => resolveStage('t2', def, breakpoint, config),
