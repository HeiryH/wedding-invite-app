...
import { T3_STAGES, T3_ASSETS } from './data/t3Stages';
...
const ENGINE: Engine = { assetRoot: T3_ASSETS, assetSizes: {}, slotRegistry: {} };

/**
 * Renders the decorative layer overlay for one Template 3 section. Reads the couple's
 * `t3.layout.<breakpoint>.<stageId>` delta straight from customConfig (the Adjust dock in the
...
export default function SectionOverlay({
  ...
  const def = T3_STAGES[stageId];
  const { layers } = useMemo(
    () => resolveStage('t3', def, breakpoint, config),
