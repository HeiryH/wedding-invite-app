...
/**
 * Turns Template 5's `anchor` layers into an inline style to merge onto the real DOM element they
 ...
 * to plain (non-framer-motion) elements, so there's no wrapper and no animation conflict.
 *
 * Returns `anchor(stageId, elementId, baseStyle?)` → a merged CSSProperties. `translate` is a
...
export function useAnchors(
  ...
  const byStage = useMemo(() => {
    ...
    for (const id of Object.keys(T5_STAGES)) {
      map[id] = resolveStage('t5', T5_STAGES[id], breakpoint, config).layers;
    }
    return map;
  }, [config, breakpoint]);
